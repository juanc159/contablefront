...
export const useCrudTypesReceiptInvoiceStore = defineStore('useCrudTypesReceiptInvoiceStore', {
  ...
  actions: {
    ...
    async fetchInfoTypesReceiptInvoices(id: number): Promise<void> {
      const preload=  usePreloadStore()
      if (id !== this.typeReceiptInvoiceData.id) {
        preload.isLoading = true
        await axiosIns
          .get(`/invoice-info/${id}`)
          .then(async (result) => {
        ...
        for (const key in this.typeReceiptInvoiceData) {
          this.formulario[key] = this.typeReceiptInvoiceData[key]
        }
      }
    },
